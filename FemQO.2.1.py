...
change_front_back_card_msg = "canvien entre reactius i producte."
...
def create_flashcard_dictionary():
    ...
    for name, pdf in flashcard_pdfs:
        ...
        for number in range(doc.pageCount//2-1):
            flashcard = {
                "QO": name,
                "index": number,
                "flashcard_front": "fc_"+name+"_"+str(number)+"_front.jpg",
                "flashcard_rear": "fc_"+name+"_"+str(number)+"_rear.jpg",
                "score_front": 0,
                "score_rear": 0,
                }
...
def show_flashcard_in_screen(photo_name, front_rear, anki_back_card=False, arrows=(0, 0)):
    ...
    photo_name = photo_name.strip("_front.jpg")
    photo_name = photo_name.strip("_rear.jpg")
    ...
    photo_output = os.path.join(flashcards_docs, "photo.jpg")
    pix.writePNG(photo_output)
    load_photo = pygame.image.load(photo_output)
    
    flashcard_width, flashcard_height = Image.open(photo_output).size
...
def IsOverFlashcard(pos):
    flashcard_0 = Image.open(os.path.join(flashcards_docs,"photo.jpg"))
...
def Welcome_screen(GameLoop):
    ...
    button_R = draw_button_and_message("R", "Repassar les cartes en ordre aleatori (random).", rectangle = [40,75] )
    button_A = draw_button_and_message("A", "Auto-avaluació (les cartes que menys et saps surten més sovint).", rectangle = [40,100] )
    
    write_message("Utilitza les fletxes per moure't entre les cartes", rectangle=[20,130], font=font_14_bold)
    ...
    draw_button_and_message("↓", "canvien de carta (només aplicable als modes Y i R).", rectangle = [53, 180], button_font=font_12, small=True)
...
def Ankistyle_review(GameLoop):
    ...
    def Anki_help(decision, Repas, GameLoop):
        ...
        draw_button_and_message("←", "canvien entre la part del davant i darrere de la carta.", rectangle = [43, row], button_font=font_12, text_font=font_16, small=True)

        write_message("Prem la tecla corresponent depenent de la dificultat de la carta:", rectangle=[20,row+25])
        row = 50
        draw_button_and_message("1", "Fallat", rectangle=[30,row+20], small=True)
        write_message("Quan has girat la carta, no era el que esperaves", rectangle=[105, row+21], font=font_14)
        draw_button_and_message("2", "Difícil", rectangle=[30,row+40], small=True)
        write_message("Quan has girat la carta, ha sortit el que t'esperaves, però no n'estaves segur/a.", rectangle=[105, row+41], font=font_14)
        draw_button_and_message("3", "Mitjà", rectangle=[30,row+60], small=True)
        write_message("Quan has girat la carta, ha sortit el que esperaves, però no t'ha vingut de forma fluïda.", rectangle=[105, row+61], font=font_14)
        draw_button_and_message("4", "Fàcil", rectangle=[30,row+80], small=True)
        write_message("Quan has girat la carta, era el que esperaves, i ho has recordat instantàniament.", rectangle=[105, row+81], font=font_14)
        draw_button_and_message("B", "Borrar", rectangle=[30,row+100], small=True)
        write_message("Prem si no vols tornar a veure la carta. Només ho podràs desfer fent un reset del progrés.", rectangle=[105, row+101], font=font_14)
        
        button_H = draw_button_and_message("H", "Retornar a l'estudi de les cartes", rectangle = [20,screen_height-55], text_font = font_14)
    ...
    def initialpage_general_screen():
        ...
        #Instructions
        row=230
        write_message("Instruccions a l'hora d'estudiar les cartes:", rectangle=[20,row-55], font=font_14_bold)
       
        draw_button_and_message("→", "", rectangle = [20,row-30], button_font=font_12, small=True)  
        draw_button_and_message("←", "canvien entre la part del davant i darrere de la carta.", rectangle = [43, row-30], button_font=font_12, text_font=font_16, small=True)

        write_message("Prem la tecla corresponent depenent de la dificultat de la carta:", rectangle=[20,row-5])
    ...
    def initialpage_2nd_decision(Repas, GameLoop, button_R, button_Q):
        def initialpage_2nd_decision_screen():
            ...
            write_message("Quantes cartes vols repassar?", rectangle=[20,60], font=font_14_bold)
            write_message("Escriu el nombre i prem Enter.", rectangle=[220,65], font=font_14)
            write_message("per estudiar el valor per defecte: 50 cartes", rectangle=[70,85], font=font_12)
    ...
    def review_flashcards_Ankistyle(Repas, GameLoop):
        ...
        while Repas:
            for card in set_cards:
                ...
                decision = False
                if TypeOfAnkiStudy == "Front_rear":
                    front_rear = "_front.jpg"
                    
                elif TypeOfAnkiStudy == "Rear_front":
                    front_rear = "_rear.jpg"
                elif TypeOfAnkiStudy == "Mixed":
                    if "front.jpg" in card:
                        front_rear = "_front.jpg"
                        score = "score_front"
                        card = card.replace("_front.jpg", "")
                    if "rear.jpg" in card:
                        front_rear = "_rear.jpg"
                        score = "score_rear"
                        card = card.replace("_rear.jpg", "")
                
                if front_rear == "_front.jpg":
                    InitialPosition = "_front.jpg"
                else:
                    InitialPosition = "_rear.jpg"
                
                while decision is False:
                    photo = card+front_rear
                    
                    if InitialPosition == front_rear:             
                        show_flashcard_in_screen(photo, front_rear)
                    else:
                        show_flashcard_in_screen(photo, front_rear, anki_back_card=True)
                    ...
                    for event in pygame.event.get():
                        ...
                        if event.type == pygame.QUIT:
                            ...
                            flashcard_dict[card][score] = 1000
                            decision = True
                        elif event.type == pygame.MOUSEBUTTONUP and IsOverFlashcard(pos):
                            if front_rear == "_front.jpg":
                               front_rear = "_rear.jpg"
                            else:
                                front_rear = "_front.jpg"
...
def study_flashcards(StudyType, GameLoop):
    ...
    while Repas:
        ...
        button_up, button_down = show_flashcard_in_screen(photo, front_rear, arrows=(button_up, button_down))
